...
class TTS_Config:
    default_configs = {
        ...
        "v3": {
            ...
            "is_half": False,
            "version": "v3",
            "t2s_weights_path": "/workspace/GPT-SoVITS/GPT_SoVITS/pretrained_models/s1v3.ckpt",
            "vits_weights_path": "/workspace/GPT-SoVITS/GPT_SoVITS/pretrained_models/s2Gv3.pth",
            "cnhuhbert_base_path": "/workspace/GPT-SoVITS/GPT_SoVITS/pretrained_models/chinese-hubert-base",
            "bert_base_path": "/workspace/GPT-SoVITS/GPT_SoVITS/pretrained_models/chinese-roberta-wwm-ext-large",
        },
        "v4": {
            ...
            "version": "v4",
            "t2s_weights_path": "/workspace/GPT-SoVITS/GPT_SoVITS/pretrained_models/s1v3.ckpt",
            "vits_weights_path": "/workspace/GPT-SoVITS/GPT_SoVITS/pretrained_models/gsv-v4-pretrained/s2Gv4.pth",
            "cnhuhbert_base_path": "/workspace/GPT-SoVITS/GPT_SoVITS/pretrained_models/chinese-hubert-base",
            "bert_base_path": "/workspace/GPT-SoVITS/GPT_SoVITS/pretrained_models/chinese-roberta-wwm-ext-large",
        },
        "v2Pro": {
            ...
            "version": "v2Pro",
            "t2s_weights_path": "/workspace/GPT-SoVITS/GPT_SoVITS/pretrained_models/s1v3.ckpt",
            "vits_weights_path": "/workspace/GPT-SoVITS/GPT_SoVITS/pretrained_models/v2Pro/s2Gv2Pro.pth",
            "cnhuhbert_base_path": "/workspace/GPT-SoVITS/GPT_SoVITS/pretrained_models/chinese-hubert-base",
            "bert_base_path": "/workspace/GPT-SoVITS/GPT_SoVITS/pretrained_models/chinese-roberta-wwm-ext-large",
        },
        "v2ProPlus": {
            ...
            "version": "v2ProPlus",
            "t2s_weights_path": "/workspace/GPT-SoVITS/GPT_SoVITS/pretrained_models/s1v3.ckpt",
            "vits_weights_path": "/workspace/GPT-SoVITS/GPT_SoVITS/pretrained_models/v2Pro/s2Gv2ProPlus.pth",
            "cnhuhbert_base_path": "/workspace/GPT-SoVITS/GPT_SoVITS/pretrained_models/chinese-hubert-base",
            "bert_base_path": "/workspace/GPT-SoVITS/GPT_SoVITS/pretrained_models/chinese-roberta-wwm-ext-large",
        },
    }
...
class TTS:
    ...
    def init_vocoder(self, version: str):
        if version == "v3":
            ...
            state_dict_g = torch.load(
                "%s/GPT-SoVITS/GPT_SoVITS/pretrained_models/gsv-v4-pretrained/vocoder.pth" % (now_dir,),
                map_location="cpu",
                weights_only=False,
            )
    ...
    def audio_postprocess(
        ...
        if inputs is not None:
            loudness_boost = inputs.get("loudness_boost", False)
            gain = inputs.get("gain", 0.0)
            normalize = inputs.get("normalize", False)
            energy_scale = inputs.get("energy_scale", 1.0)
            volume_scale = inputs.get("volume_scale", 1.0)
            strain_effect = inputs.get("strain_effect", 0.0)
            is_int32 = inputs.get("is_int32", False)
        else:
        ...
        for i, batch in enumerate(audio):
            for j, audio_fragment in enumerate(batch):
                ...
                if inputs is not None and inputs.get("postprocess", False):
                    if gain != 0.0:
                        gain_linear = 10 ** (gain / 20.0)
                        audio_fragment *= gain_linear
                    if volume_scale != 1.0:
                        audio_fragment *= volume_scale
                    if energy_scale != 1.0:
                        rms = torch.sqrt(torch.mean(audio_fragment ** 2))
                        if rms > 0:
                            audio_fragment *= energy_scale / rms
                    if normalize:
                        max_amplitude = torch.abs(audio_fragment).max()
                        if max_amplitude > 0:
                            audio_fragment /= max_amplitude
                    if loudness_boost:
                        audio_fragment *= 1.5
                    if strain_effect != 0.0:
                        pitch_shift = torchaudio.transforms.PitchShift(sample_rate=sr, n_steps=strain_effect)
                        audio_fragment = pitch_shift(audio_fragment.unsqueeze(0)).squeeze(0)
